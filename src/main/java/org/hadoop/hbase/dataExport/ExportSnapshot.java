package org.hadoop.hbase.dataExport;
...
public class ExportSnapshot extends Configured implements Tool {
    ...
    private static final String CONF_SNAPSHOT_OLD_NAME = "snapshot.export.format.snapshot.old.name";
    private static final String CONF_SNAPSHOT_DIR = "snapshot.export.format.snapshot.dir";
    private static final String CONF_SNAPSHOT_OLD_DIR = "snapshot.export.format.snapshot.old.dir";
        ...
        private Path getOutputPath(final SnapshotFileInfo inputInfo) throws IOException {
            ...
            switch (inputInfo.getType()) {
                case HFILE:
                    ...
                    LOG.info("测试日志getOutputPath：" + path.getName());
                    break;
                case WAL:
    ...
    private static class ExportSnapshotInputFormat extends InputFormat<BytesWritable, NullWritable> {
        ...
        public List<InputSplit> getSplits(JobContext context) throws IOException, InterruptedException {
            ...
            List<Pair<SnapshotFileInfo, Long>> snapshotFiles = new ArrayList<>();
            List<Pair<SnapshotFileInfo, Long>> origSnapshotFiles = getSnapshotFiles(conf, fs, snapshotDir);
    ...
    private void runCopyJob(final Path inputRoot, final Path outputRoot,
                            ...
                            final int mappers, final int bandwidthMB, final String snapshotOldName, final Path snapShotOldDir)
        ...
        if(snapshotOldName != null) {
            conf.set(CONF_SNAPSHOT_OLD_NAME, snapshotOldName);
        }
        if(snapShotOldDir != null) {
            conf.set(CONF_SNAPSHOT_OLD_DIR, snapShotOldDir.toString());
        }

        Job job = new Job(conf);
        job.setJobName("ExportSnapshot-" + snapshotName);
        if(snapshotOldName != null) {
            job.setJobName("ExportSnapshotIncremental-" + snapshotName);
        }
    ...
    public int run(String[] args) throws IOException {
        ...
        Path snapShotOldDir = null;
        if(snapshotOldName != null) {
            snapShotOldDir = SnapshotDescriptionUtils.getCompletedSnapshotDir(snapshotOldName, inputRoot);
        }
        ...
        try {
            runCopyJob(inputRoot, outputRoot, snapshotName, snapshotDir, verifyChecksum,
                    filesUser, filesGroup, filesMode, mappers, bandwidthMB, snapshotOldName, snapShotOldDir);
            ...
            if (verifyTarget) {
                ...
                //TODO 如果全量导出则校验
                if(snapshotOldName == null || snapshotOldName.length() == 0) {
    ...
    private void printUsageAndExit() {
        ...
        System.err.println("  -snapshot-old NAME      此快照用于导出增量数据对比.");
